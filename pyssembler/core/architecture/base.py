...
if typing.TYPE_CHECKING:
    ...
    from pyssembler.core.architecture.program import AssemblyFile, SourceLine
...
CPUT = typing.TypeVar('CPUT', bound='PyssemblerCPU')


class PyssemblerAssembler(typing.Generic[CPUT], LoggableMixin):
    def __init__(self, cpu: CPUT):
        self._cpu: CPUT = cpu
        self._log = self.get_logger(level=logging.INFO)
...
class PyssemblerCPU(ABC, LoggableMixin):
    ...
    @abstractmethod
    def get_current_program(self) -> typing.Optional[PyssemblerProgram]:
        ...
    ...
    @abstractmethod
    def get_current_statement(self) -> TokenizedStatement:
        ...
...
# Instructions
# ----------------------------------------------------------------------------
class PyssemblerBasicInstruction(typing.Generic[CPUT], LoggableMixin):
...
InstrT = typing.TypeVar('InstrT', bound=PyssemblerBasicInstruction)


class PyssemblerInstructionSet(typing.Generic[InstrT], LoggableMixin):
    ...
    def __init__(self) -> None:
        self._basic_instrs: typing.List[InstrT] = []
        self._log = self.get_logger(level=logging.INFO)

    def add_instruction(self, instr: InstrT) -> None:
        self._log.debug(f'Adding Instruction: {instr.mnemonic}...')
        self._basic_instrs.append(instr)

    def match_instruction(self, statement: TokenizedStatement) -> InstrT:
