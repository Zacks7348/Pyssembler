...
from pyssembler.core.architecture import *
from pyssembler.utils import numeric, str_utils
...
class MIPSAssembler(PyssemblerAssembler[MIPSCPU]):
    ...
    def assemble_single_instruction(
            self,
            instr_txt: str,
            asm_file: AssemblyFile,
    ) -> MIPSStatement:
        """Assembles a single instruction to be executed in a debug context.

        The instruction is not written into simulated memory.
        """
        program = self._cpu.get_current_program()
        original_segment = self._ctx.current_segment
        original_um = self._ctx.update_memory
        original_pi = self._ctx.process_includes

        self._ctx.update_memory = False
        self._ctx.process_includes = False
        ...
        statements = self.assemble_from_tokens(program, raw_statements, start_segment=Segment.KTEXT)

        self._ctx.current_segment = original_segment
        self._ctx.update_memory = original_um
        self._ctx.process_includes = original_pi
    ...
    def assemble_from_tokens(
        ...
        self._ctx.global_symbol_list.clear()
