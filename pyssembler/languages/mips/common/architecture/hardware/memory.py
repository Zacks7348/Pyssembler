...
from pyssembler.utils import LoggableMixin, numeric

if typing.TYPE_CHECKING:
    from ..statement import MIPSStatement


class MIPSMemorySegment(LoggableMixin):
    ...
    def __init__(
        ...
        self._log = self.get_logger(suffix=self.name)
    ...
    def update_range(self, lower: int = None, upper: int = None):
        ...
        self.validate_address_range()
        self._log.debug(
            f'Segment range was updated:\n'
            f'\tOriginal Range: {original_lower} - {original_upper}\n'
            f'\tNew Range:      {self._lower_address} - {self._upper_address}'
        )
    ...
    def write_byte(self, addr: int, val: int) -> None:
        ...
        self._log.debug(f'Writing byte to address {addr} (0x{addr:08x}): {val} (0x{val:02x})')
...
class MIPSTextMemorySegment(MIPSMemorySegment):
    ...
    def write_instruction(self, addr: int, statement: MIPSStatement) -> None:
        self._log.debug(f'Writing instruction to address {addr} (0x{addr:08x}): {statement}')
        self._validate_address(addr)
        self._instr_memory[addr] = statement

        # TODO: Write binary to memory


class MIPSMemory(LoggableMixin):
    ...
    def read_bytes(
        ...
        By default, all of simulated memory is accessible. Operating modes can be
    ...
    def write_bytes(
        ...
        :param program_segment: Optional, segment of program where memory access is occurring
                                (default = ALL).
        """
        self._log.debug(f'Writing {num_bytes} byte(s) to address {start_addr} (0x{start_addr:08x}): {val} (0x{val:08x})')
