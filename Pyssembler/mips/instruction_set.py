...
class InstructionSet:
    ...
    def best_match(self, instr: int):
        ...
        best_match = None
        best_match_num = 0
        instr_op = (instr & self.OP_MASK[0]) >> self.OP_MASK[1]
    ...
    def populate(self):
        ...
        TODO: descriptions for all instructions, instr sim functions
        """
    ...
    def get_i16(self, instr: int, signed=False) -> int:
        """
        Return 16-bit immediate from instr
        """
        return Integer.get_bits(instr, 0, 15, signed=signed)

    # The following functions are used to execute the different mips instructions

    def simulate_instr(self, instr: int):
        match = self.best_match(instr)
        self.instructions[match]['sim'](instr)

    def _lb(self, instr: int):
        """
        Simulate a lb instruction
        """
        base = self.get_reg1(instr)
        rt = self.get_reg2(instr)
        i16 = self.get_i16(instr, signed=True)
        RF.write(rt, MEM.read_byte(base + i16, signed=True))
