...
#
INSTR_PSUEDO = ["beqz", "blt", "bge", "bgt", "ble", "clear", "la", "li", "move", "mul"]
INSTR_PARENTHESIS = ["lb", "lw", "sb", "sw"]
INSTR_BRANCH = ["beq", "bgez", "bgezal", "bgtz", "blez", "bltz", "bltzal", "bne"]
INSTR_J = ["j", "jal"]
...
INSTR_013 = ["addi", "addiu", "andi", "ori", "slti", "sltiu", "xori"]
...
def mips_to_binary(code):
    ...
    for cnt, line in enumerate(code):
        mips = line[0].split()
        if ":" in mips[0]:
            mips.pop(0)
        instr = mips[0]
            ...
            if instr == "noop":
                ...
                if reg1 is None:
                    ...
                    raise InvalidRegisterError(
                        line, code[cnt][1], mips[2].split("(")[1].replace(")", "")
                    )
...
def mips_to_hex(code):
    bins = mips_to_binary(code)
    if not bins[1]:
        return bins
    result = []
    for b in bins[0]:
        n = int(str(b), 2)
        result.append(f"{n:08x}")
    return (result, True)
