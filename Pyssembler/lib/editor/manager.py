...
from Pyssembler.environment.mips_translator import mips_to_binary, mips_to_hex
...
class Manager:
    ...
    def highlight_syntax(self, line=True):
        if Settings().editor["highlight-syntax"]:
    ...
    def link_update_functions(self, function):
    ...
    def mips_to_hex(self):
        self.app.console.info("Translating mips...")
        code = self.app.get_text_editor().splitlines()
        code = mips_to_hex(code)
        if code[1]:
            self.app.console.info("Successfully translated MIPS to hex!")
            return code[0]
        else:
            self.app.console.error(str(code[0]))
            return None
    ...
    def translate(self, mode):
        ...
        if mode == "bin":
            ...
            code = mips_to_hex(code)
        if code[1]:
            self.app.console.info("Successfully translated MIPS to {}!".format(mode))
            return code[0]
        else:
            self.app.console.error(str(code[0]))
            return None
